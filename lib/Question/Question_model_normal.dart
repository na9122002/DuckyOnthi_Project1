...
import 'package:travo_app_source/EnglishClassUnivercity/model_list.dart';
import 'package:travo_app_source/EnglishClassUnivercity/question_model.dart';
import 'package:travo_app_source/Question/ResultScreen.dart';
import 'package:travo_app_source/helpers/asset_helper.dart';

class QuesionNormalModel extends StatefulWidget {
  const QuesionNormalModel(
      {Key? key, required this.topic, required this.subject})
      : super(key: key);
  final String topic;
  final String subject;
  @override
  State<QuesionNormalModel> createState() => _QuesionNormalModelState();
}

class _QuesionNormalModelState extends State<QuesionNormalModel> {
  ...
  _CreatNopBai(int quantityQ, dynamic snapshot, dynamic OldHour, dynamic time) {
  ...
  _CreatExercisesDone(dynamic OldHour, dynamic numbertrueAnswer,
      dynamic numberflaseAnwer, dynamic _time, dynamic _minutes) async {
    int trueAnswerNow = 0;
    int falseAnwerNow = 0;
    for (int i = 0; i < _models!.length; i++) {
      if (Choose[i] == _models![i].trueAnswer && _models![i].id != '-') {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < _models!.length; i++) {
      if (_models![i].falseAnwer1 != '-') {
        ...
        ListTrueAnswer.add(_models![i].trueAnswer);
      }
    }
    return FutureBuilder<User?>(
        future: readDATA(MonthTime, widget.subject),
              ...
              actions: [
                TextButton(
                  ...
                  onPressed: () {
                    ...
                    _CreatNopBai(quantityQ, snapshot, OldHour, time);
                    _CreatExercisesDone(
                        OldHour, trueAnswer, falseAnwer, time, Minutes);
                    StreamBuilder(
                        ...
                        child: Container(
                          ...
                          child: TweenAnimationBuilder<Duration>(
                              ...
                              onEnd: () {
                                String timeWorkDone = '${Minutes} : ${Seconds}';
                                _CreatNopBai(
                                    quantityQ, snapshot, OldHour, time);
                                _CreatExercisesDone(OldHour, trueAnswer,
                                    falseAnwer, time, Minutes);
                                createWorkDone(
                                    snapshot,
                                    widget.subject,
                                    timeWorkDone,
